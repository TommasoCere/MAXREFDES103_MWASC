...
class ArchiveFragment : RecyclerViewClickListener, Fragment(),
    ...
    override fun onAddInfoClicked(file: File) {
            ...
            .setPositiveButton("OK", DialogInterface.OnClickListener { dialog, which ->
                ...
                ioThread {
                    val renamedFile = File(
                        file.parentFile,
                        File.separator + file.nameWithoutExtension + "_sao2_$saO2Value." + file.extension
                    )
    ...
    private fun runSleepAlgorithm(file: File): Boolean {
        ...
        if (!SQA_OUTPUT_DIRECTORY.exists()) {
            SQA_OUTPUT_DIRECTORY.mkdirs()
        }

        val outputFile = File(
            SQA_OUTPUT_DIRECTORY,
            file.nameWithoutExtension + "_${user.username}.${file.extension}"
        )
        ...
        if (user.sleepRestingHr == 0) {
            ...
            val oneHzFile = File(
                ...
                "${File.separator}${file.nameWithoutExtension}${ONE_HZ_SUFFIX}.csv"
            )
