            ...
            for (line in lines) {
                ...
                try{
                    val sleepPhasesOutputProcessed = if(arr.size > 10){
                        arr[SLEEP_sleepPhasesOutputProcessed.ordinal].trim().toIntOrNull() ?: arr[SLEEP_sleepPhasesOutput.ordinal].trim().toInt()
                    }else{
                    ...
                    sleepList.add(
                        Sleep(
                            ...
                            sleepPhasesReady = arr[SLEEP_sleepPhasesReady.ordinal].trim().toInt(),
                            sleepPhasesOutput = arr[SLEEP_sleepPhasesOutput.ordinal].trim().toInt(),
                            encodedOutput_sleepPhaseOutput = arr[SLEEP_encodedOutput_sleepPhaseOutput.ordinal].trim().toInt(),
                            encodedOutput_duration = arr[SLEEP_encodedOutput_duration.ordinal].trim().toInt(),
                            encodedOutput_needsStorage = arr[SLEEP_encodedOutput_needsStorage.ordinal].trim().toBoolean(),
